import { getGraduatedTokens, isTokenVerified } from '@/app/lib/birdeye';
import { NextResponse } from 'next/server';

export async function GET() {
  try {
    const tokens = await getGraduatedTokens(30);
    ...
    console.error('Failed to fetch graduated tokens:', error);
