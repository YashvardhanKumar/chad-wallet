    ...
    <div className="fixed inset-x-0 top-0 z-[110] flex justify-center px-4 pt-[calc(env(safe-area-inset-top)+1rem)]">
      <div ref={panelRef} className="w-full max-w-[min(28rem,calc(100vw-2rem))]">
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="flex h-12 items-center gap-3 rounded-t-[1.75rem] border border-[color-mix(in_srgb,var(--cw-header-border)_72%,transparent)] border-b-transparent bg-[color-mix(in_srgb,var(--cw-surface)_82%,var(--cw-bg-app))] px-4 text-[var(--cw-header-text-primary)] shadow-[0_22px_36px_-30px_rgba(0,0,0,0.95)] transition-[border-color,box-shadow,border-radius] duration-200 focus-within:border-[color-mix(in_srgb,var(--cw-hover-accent)_28%,var(--cw-header-border))] focus-within:shadow-[0_26px_44px_-30px_rgba(0,0,0,0.98)]">
            <svg stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" className="h-5 w-5 shrink-0 text-[var(--cw-header-text-secondary)]" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
            ...
            <input
              ...
              className="min-w-0 flex-1 border-0 bg-transparent p-0 text-sm font-semibold text-[var(--cw-header-text-primary)] placeholder:text-[color-mix(in_srgb,var(--cw-header-text-secondary)_88%,transparent)] focus:outline-none focus:ring-0 sm:text-base"
              value={query}
            ...
            <button
              ...
              onClick={onClose}
              className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[var(--cw-header-text-primary)] transition-colors hover:bg-[color-mix(in_srgb,var(--cw-header-text-primary)_10%,transparent)]"
            >
        ...
        >
          <div className="max-h-[min(28rem,calc(100vh-10rem))] overflow-y-auto p-2">
            <div className="grid gap-1">
              {results.length === 0 ? (
                <div className="px-3 py-8 text-center text-sm text-[var(--cw-text-secondary)]">
                  No tokens found
                </div>
              ) : (
                results.map((token) => (
